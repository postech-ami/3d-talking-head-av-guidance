...
def face_vertices_func(vertices, faces):
    """ 
    :param vertices: [batch size, number of vertices, 3]
    :param faces: [batch size, number of faces, 3]
...
class StandardRasterizer(nn.Module):
    ...
    def forward(self, vertices, faces, attributes=None, h=None, w=None):
        ...
        f_vs = face_vertices_func(vertices, faces)
...
class SRenderY(nn.Module):
    def __init__(self, image_size, obj_filename, uv_size=256, rasterizer_type='pytorch3d'):
        ...
        face_uvcoords = face_vertices_func(uvcoords, uvfaces)
        ...
        face_colors = face_vertices_func(colors, faces)
    ...
    def forward(self, vertices, transformed_vertices, albedos, lights=None, h=None, w=None, light_type='point', background=None):
        ...
        # attributes
        face_vertices = face_vertices_func(vertices, self.faces.expand(batch_size, -1, -1))
        normals = vertex_normals(vertices, self.faces.expand(batch_size, -1, -1))
        face_normals = face_vertices_func(normals, self.faces.expand(batch_size, -1, -1))
        transformed_normals = vertex_normals(transformed_vertices, self.faces.expand(batch_size, -1, -1))
        transformed_face_normals = face_vertices_func(transformed_normals, self.faces.expand(batch_size, -1, -1))
    ...
    def render_shape(self, vertices, transformed_vertices, colors = None, images=None, detail_normal_images=None, 
                lights=None, return_grid=False, uv_detail_normals=None, h=None, w=None, transparent_coeffi=1.0):
        '''
        rendering shape with detail normal map

        Args:
            vertices (torch.Tensor) : shape - (Batch, Num of vertices 5023, Dim 3)
            transformed_vertices (torch.Tensor) : shape - (Batch, Num of vertices 5023, Dim 3)
            colors (torch.Tensor) : shape - (batch, num_vertices 5023, )
            images (torch.Tensor) : shape - (batch, channel 3, height 224, width 224)
        '''
        batch_size = vertices.shape[0]
        # set lighting
        if lights is None:
        ...
        if colors is None:
            ...
            colors = face_vertices_func(colors, self.faces.expand(batch_size, -1, -1))

        attributes = torch.cat(
            [colors, transformed_face_normals.detach(), face_vertices.detach(), face_normals, self.face_uvcoords.expand(batch_size, -1, -1, -1)], 
            -1
        )
        # rasterize
        rendering = self.rasterizer(transformed_vertices, self.faces.expand(batch_size, -1, -1), attributes, h, w)
        ...
        alpha_images = alpha_images*transparent_coeffi
        if images is None:
    ...
    def render_depth(self, transformed_vertices):
        ...
        z = z/z.max()
        # Attributes
        attributes = face_vertices_func(z, self.faces.expand(batch_size, -1, -1))
        # rasterize
        transformed_vertices[:,:,2] = transformed_vertices[:,:,2] + 10
    ...
    def render_colors(self, transformed_vertices, colors):
        ...
        # Attributes
        attributes = face_vertices_func(colors, self.faces.expand(batch_size, -1, -1))
        # rasterize
        rendering = self.rasterizer(transformed_vertices, self.faces.expand(batch_size, -1, -1), attributes)
        ####
