...
def init_biased_mask(n_head, max_seq_len, period):
    ...
    # bias = torch.div(torch.arange(start=0, end=max_seq_len, step=period).unsqueeze(1).repeat(1,period).view(-1), period, rounding_mode='trunc')
...
class Faceformer(nn.Module):
    ...
    def forward(self, audio, template, vertice, one_hot, teacher_forcing=True):
        ...
        loss = self.criterion(vertice_out, vertice) # (batch, seq_len, V*3)
        loss = torch.mean(loss)

        return loss, vertice, vertice_out
